...
module UnitOfWorkSqlRunnerTransactionTests =
    ...
    type DummyUnitOfWork(ctxFactory: IDbContextFactory<DummyDbContext>) =
        ...
        let rec findContextField(obj, t: Type) =
            let field = t.GetField("_dbContext", BindingFlags.NonPublic ||| BindingFlags.Instance)
            if field <> null then
                field.GetValue(obj) :?> DummyDbContext
            else
                let baseType = t.BaseType
                if baseType <> null then
                    findContextField(obj, baseType)
                else
                    failwith "Unable to access _dbContext field"
        member this.PrivateContext = findContextField(this, this.GetType())
