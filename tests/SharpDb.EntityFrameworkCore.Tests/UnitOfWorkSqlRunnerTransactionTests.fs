...
module UnitOfWorkSqlRunnerTransactionTests =
    ...
    let ``EfcSqlRunner SqlExecuteAsync in UoW transaction rolls back on exception`` () =
        ...
        try
            ...
            Assert.Equal(0, ctx.Set<DummyEntity>().CountAsync(fun e -> e.Name = name).Result)
        finally
            ctx.Dispose()
            conn.Close()
            conn.Dispose()
    ...
    let ``Nested UoW transactions with EfcSqlRunner roll back inner only`` () =
        ...
        try
            ...
            Assert.True(result.IsSuccess)
            let countOuter = ctx.Set<DummyEntity>().CountAsync(fun e -> e.Name = nameOuter).Result
            let countInner = ctx.Set<DummyEntity>().CountAsync(fun e -> e.Name = nameInner).Result
